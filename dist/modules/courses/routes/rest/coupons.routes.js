 ...
 */
router.get('/', (0, roles_1.requireRole)('ADMIN'), async (req, res) => {
    try {
 ...
 */
router.post('/', (0, roles_1.requireRole)('ADMIN'), async (req, res) => {
    try {
...
router.post('/apply', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const result = await couponsService.applyCoupon(code, userId, amount, applicableTo, courseId, bundleId);
        if (!result.valid) {
