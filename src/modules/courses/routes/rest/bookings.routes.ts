 ...
 *     tags: [Bookings]
 */
router.post('/teachers/:teacherId/availability', requireAuth, requireRole('TEACHER'), async (req, res) => {
  try {
    const { teacherId } = req.params;
 ...
 *     tags: [Bookings]
 */
router.delete('/teachers/:teacherId/availability/:availabilityId', requireAuth, requireRole('TEACHER'), async (req, res) => {
  try {
...
router.get('/lesson-slots', requireAuth, async (req, res) => {
  try {
    ...
    const slots = await prisma.moduleSlot.findMany({
      where,
      include: {
        teacher: {
...
router.post('/lesson-slots/:slotId/bookings', requireAuth, async (req, res) => {
  try {
    ...
    const slot = await prisma.moduleSlot.findUnique({
      where: { id: slotId },
