...
router.post('/',
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({ error: (error as Error).message || 'Failed to create whiteboard' });
    }
  }
);
...
router.get('/:whiteboardId',
  ...
  async (req, res) => {
    try {
      ...
      res.status(404).json({ error: (error as Error).message || 'Whiteboard not found' });
    }
  }
);
...
router.put('/:whiteboardId',
  ...
  async (req, res) => {
    try {
      ...
      res.status(403).json({ error: (error as Error).message || 'Failed to update whiteboard' });
    }
  }
);
...
router.delete('/:whiteboardId',
  ...
  async (req, res) => {
    try {
      ...
      res.status(403).json({ error: (error as Error).message || 'Failed to delete whiteboard' });
    }
  }
);
...
router.post('/:whiteboardId/actions',
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({ error: (error as Error).message || 'Failed to add action' });
    }
  }
);
...
router.post('/:whiteboardId/clear',
  ...
  async (req, res) => {
    try {
      ...
      res.status(403).json({ error: (error as Error).message || 'Failed to clear whiteboard' });
    }
  }
);
